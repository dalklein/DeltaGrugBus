...
from grugbus.devices import Delta_ExTLUS, Acrel_AGF_AE_D
...
########################################################################################
#
#       Delta ExTL-US inverter - AC coupled behind SMA SI6048US, as if the SMA is the 'grid'
#         the Delta ExTL-US is set for self consumption mode, which generally controls the
#         power flow required to/from the SMA inverters to zero.
#
#       This class communicates with the inverter's COM port (we'll see if we can later)
#           
#
########################################################################################
class Delta( grugbus.SlaveDevice ):
    def __init__( self, modbus, modbus_addr, key, name, local_meter, fake_meter, mqtt, mqtt_topic ):
        super().__init__( modbus, modbus_addr, key, name, Delta_ExTLUS.MakeRegisters() )

        self.local_meter = local_meter        # Acrel_AGF_AE_D, on AC1 port from SMA SI6048US
